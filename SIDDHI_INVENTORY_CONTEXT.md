...
- **Design photo lookup:** a persistent search box in the top bar, plus a live (oninput, debounced) lookup on the design-code field inside every entry row. Every outcome is visible — real photo, "found but photo not synced from that device," or "not in Bangle Tracker" — never a silent no-op. **Bangle Tracker's design *name* is deliberately never shown anywhere** (search box, wizard photo confirm) — the owner hasn't finalized proper names against codes yet, so only the code + photo display until that's done. Don't reintroduce `hit.name` display without checking with the owner first.
- **Every design photo is tap-to-zoom:** search-box result, entry-row thumbnail, and wizard photo confirm all open a full-screen lightbox on click (double-tap to zoom 2x, pinch-zoom also works). One delegated click handler (`img.swatch`, `.lookup-pop img`, `.wiz-photo img`) covers all three locations — any new photo display spot just needs one of those selectors/classes to get zoom for free.
- **Aasan Entry (guided wizard):** a full-screen, one-question-at-a-time alternate entry mode for team members uncomfortable with tables/forms — big tap targets, Hinglish only, minimal fields (no reject/customer/variety — those stay table-only). Launched from a banner at the top of Naya Entry. Shares the exact same `commitMovement()` save path as the detailed table, so it can never produce a differently-shaped record. **Supports multiple design codes per entry**, same as the table: after each code's sizes (or plating qty) are entered, it asks "Ek Aur Design Code Jodein?" (add another?) and loops back to the code step if yes — state lives in `wizState.designs[]` (an array being built up) plus `wizState.current` (the one in progress). The review screen lists every design added, and for outgoing entries offers both "Save Karein" and "Save & Chitthi Print" (matching the table's two-button pattern); incoming entries only ever get a single save button, no print.
- **Chitthi printing isn't limited to the moment of saving** — every outgoing entry in Daybook has its own "Chitthi Print Karein" button (`printMovementById(id)`, looks the movement up from `S.movements` and calls the same `printChitthi()`), so a slip can be reprinted anytime later if it was skipped originally.
...
- **Installable as an app (PWA):** registered service worker (`siddhi-inv-sw.js`, cache name currently `siddhi-inv-v2` — bump this on any change to cached assets so already-installed copies pick it up), proper manifest, and a dedicated icon (`siddhi-inv-icon-*.png` — a plain gold "SI" monogram in a thin ring). The icon went through two attempts: a bangle-ring motif first, which the owner said still read as too visually similar to Bangle Tracker's own icon on a shared home screen — the current SI monogram is the fix, deliberately textual so it can't be confused. Install banner: native "Install Karein" button on Android/Chrome via `beforeinstallprompt`; manual Share → Add to Home Screen instructions on iOS, since no install API exists there.
- **A saved movement can be deleted** from Daybook (trash icon on each entry) — real Firebase delete, no undo, gated behind a confirm dialog naming the party/qty. This is the one exception to "append-only"; there's still no *edit* UI, only delete-and-redo.
...
**Built and tested against the real Firebase database (not just mockup):** login/session, locked-tab gating, vendor/customer sync from Bangle Tracker, Naya Entry — both the detailed table and the Aasan wizard, each supporting multiple design codes per entry, packet stacking, plating pending-reconciliation with loss tracking — Stock Kitna Hai aggregation, Opening Balance/Correction, Daybook (including per-entry delete and per-entry chitthi reprint), chitthi generation (two-copy print HTML, verified for both single- and multi-design movements — the actual browser print dialog itself is inherently a manual/visual step and wasn't machine-tested), design-photo lookup + tap-to-zoom lightbox (verified against real Bangle Tracker data — 789 designs, 1040 usable synced photos), PWA install banner and manifest/service-worker logic (the native install *prompt* itself can only be confirmed on the real HTTPS deployment, not over `file://`).
...
- Editing a saved movement in place — only delete-and-redo exists, no edit UI.
...
- Bangle Tracker design *names* aren't captured/shown yet by design — owner is still finalizing correct names against codes. Don't add name display back in until told to.
- The pending-plating reconciliation step in the wizard still only lets you pick **one** outstanding design at a time (unlike the rest of the wizard, which now supports multiple design codes per entry) — not extended in this round since it wasn't requested.
